# =============================================================================
#  匠コネクト インスタ リール用 PR動画ビルド（縦型 1080x1920・約60秒）
...
public static class ReelMaker {
  ...
  public static void Showcase(string path, string kicker, string[] head, string shotPng){
    using(var bmp=new Bitmap(W,H)) using(var g=G(bmp)){
      ...
      using(var fF=new Font("Yu Gothic UI",24,FontStyle.Bold)){ var sf=new StringFormat(){ Alignment=StringAlignment.Center }; g.DrawString("匠コネクト",fF,new SolidBrush(Sub),new RectangleF(0,H-56,W,40),sf); }
      bmp.Save(path, ImageFormat.Png);
    }
  }
...
$slides = @(
  ...
  @{ t='cover'; kicker='大工を育てる塾「一棟司塾 匠」から'; big=@('匠コネクト'); sub=@('地域の工務店どうしで','大工職人をシェアするアプリ');
     n='たくみコネクト。地域の工務店どうしで、大工職人をシェアするアプリです。' },
  @{ t='cover'; logo=$logo; kicker='「一棟司塾 匠」の工務店ネットワーク'; big=@('育てた職人を、','地域でシェア'); sub=@('塾で腕を磨いた職人を、地域でシェア。','建方・内装・階段・和室…認定資格で','技術に見合った、適正な単価に。');
     n='たくみコネクトは、いっとうつかさじゅく たくみの工務店ネットワークです。塾で腕を磨いた職人を、地域でシェア。建方、内装、階段、和室などの認定資格と連動し、技術に見合った、適正な単価を実現します。' },
  ...
  @{ t='cover'; kicker='大工を育てる塾「一棟司塾 匠」と共に'; big=@('匠コネクト'); sub=@('育てて、シェアして、','地域で仕事と職人を守る。'); footer='音声:VOICEVOX 玄野武宏 ／ 制作:オリジナル';
     n='たくみコネクト。育てて、シェアして、地域で仕事と職人を守る。いっとうつかさじゅく たくみと共に、地域の未来をつくります。' }
)

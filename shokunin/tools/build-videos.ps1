  ...
  public static void Content(string path, string kicker, string title, string[] lines, string shotPng, string charPng, int revealCount){
    ...
    using (var g = Graphics.FromImage(bmp)){
      ...
      using(var bS=new SolidBrush(Sub)){
        ...
        g.DrawString("大工コネクト", fF, bS, new RectangleF(200,676,400,32));
      }
      if(hasShot){
...
$howto = @(
  @{ cover=$true; k='かんたん使い方ガイド'; t='大工コネクト';
     l=@('こんにちは。案内役の大工「匠（たくみ）」です。','アプリの使い方を、実際の画面でご説明します。');
     n='こんにちは。だいくコネクト、案内役の大工、たくみと申します。工務店どうしで大工をシェアできるこのアプリの使い方を、実際の画面を見ながら、順番にご説明します。' },
  ...
  @{ cover=$true; k='さっそく使ってみましょう'; t='困ったときはヘルプ';
     ...
     n='困ったときは、画面上のヘルプボタンから、いつでも使い方を検索できます。また、体験モードを使えば、データが24時間で消えるお試し環境で練習できます。それでは、だいくコネクトを、ぜひご活用ください。案内役は、大工のたくみでした。' }
)

# ================= 動画② 利用価値 =================
$value = @(
  @{ cover=$true; k='地域の工務店のための'; t='大工コネクト';
     l=@('こんにちは。大工の「匠（たくみ）」です。','「人がいない」「仕事がない」という悩みを、','地域の工務店どうしで解決するアプリです。');
     n='こんにちは。大工のたくみです。忙しいときにひとがいない。暇なときに仕事がない。そんな工務店の悩みを、地域の仲間と解決するアプリ、だいくコネクトをご紹介します。' },
  @{ k='課題'; t='職人の繁閑の波';
  ...
  @{ cover=$true; k='地域で、仕事と職人を守る'; t='大工コネクト';
     l=@('信頼できる工務店ネットワークで、繁閑の波を乗り越えましょう。','体験モード（?demo=1）でお試しいただけます。');
     f='音声：VOICEVOX 玄野武宏 ／ イラスト・BGM：オリジナル';
     n='地域の信頼できる工務店ネットワークで、仕事と職人を守る。だいくコネクトで、繁閑の波を乗り越えていきましょう。体験モードでのお試しも可能です。案内役は、大工のたくみでした。' }
)
...
$caution = @(
  @{ cover=$true; k='応援を依頼する前に'; t='依頼の際の注意事項';
     ...
     n='こんにちは。だいくコネクト、案内役のたくみです。他社の大工さんに応援をお願いするときに、知っておいていただきたい注意点を、順番にご説明します。' },
  @{ k='注意点 1'; t='依頼できるのは「空き」の日だけ'; shot='reqform';
     ...
     n=@('一つ目です。',
         ...
         'おうそうだんの日は確定ではありませんので、チャットでよくご相談ください。',
  ...
  @{ cover=$true; k='注意点を守って、安心の応援を'; t='大工コネクト';

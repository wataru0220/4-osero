    ...
    using (var g = Graphics.FromImage(bmp)){
      ...
      using(var fF=new Font("Yu Gothic UI",15)){
        ...
        foreach(var ln in lines){
          ...
          y += Math.Max(44f, sz.Height+8f);
        }
        if(footer!=null && footer.Length>0) g.DrawString(footer, fF, Brushes.White, new RectangleF(70,662,900,40));
      }
      bmp.Save(path, ImageFormat.Png);
    }
  }

  public static void Content(string path, string kicker, string title, string[] lines, string shotPng, string charPng){
    using (var bmp = new Bitmap(1280,720))
    using (var g = Graphics.FromImage(bmp)){
      g.SmoothingMode = SmoothingMode.AntiAlias;
      g.TextRenderingHint = TextRenderingHint.AntiAliasGridFit;
...
function New-Voice { param([string]$Text,[string]$Path)
  ...
  $q.speedScale=1.04; $q.pitchScale=0.02; $q.intonationScale=1.12; $q.postPhonemeLength=0.3
...
function Build-Video {
  ...
  for($i=0;$i -lt $Slides.Count;$i++){
    ...
    $wav=Join-Path $work ("{0}_{1:d2}.wav" -f $Name,$i)
    ...
    if($s.cover -eq $true){
      ...
      [SlideMaker2]::Content($png,$s.k,$s.t,[string[]]$s.l,$shot,$char)
    }
    New-Voice -Text $s.n -Path $wav
    cmd /c "`"$ff`" -y -hide_banner -loglevel error -loop 1 -i `"$png`" -i `"$wav`" -af apad=pad_dur=0.8 -c:v libx264 -tune stillimage -r 15 -c:a aac -ar 44100 -b:a 128k -pix_fmt yuv420p -shortest `"$mp4`" 2>nul" | Out-Null
...
$howto = @(
  ...
  @{ k='ステップ 1'; t='ログインと工務店登録'; shot='login';
     ...
     n='ステップいち、ログインと工務店登録です。がめんしたの「自社の大工」タブを開き、メールアドレスとパスワードで新規登録します。続けて、自社の工務店名と、最初の大工をひとり登録してください。初回ログインの際は、利用規約への同意をお願いします。' },
  @{ k='ステップ 2'; t='空き予定の登録'; shot='avail';
     l=@('大工ごとにカレンダーで空き日をタップ','横にスワイプすると連続でまとめて入力できます','「まとめて見る」で自社全員の空きも確認');
     n='ステップに、空き予定の登録です。大工ごとにカレンダーを開き、空いている日をタップします。横に指でなぞれば、連続した日をまとめて入力できます。登録した空き日は、他の工務店の検索にすぐ反映されます。自社全員の空きも、カレンダーでまとめて確認できます。' },
  @{ k='ステップ 3'; t='大工をさがす'; shot='find';
     l=@('名前・得意作業・資格で検索できます','空き予定のみ・お気に入りのみで絞り込みも可能','技術力の★は利用者全員の平均です');
     n='ステップさん、大工をさがすです。検索タブでは、名前や得意作業、資格から他社の大工を探せます。空きのある大工だけに絞り込むことも可能です。星の数は利用者全員の平均評価ですので、選ぶ際の目安になります。' },
  @{ k='ステップ 4'; t='応援要請を送る'; shot='reqform';
     l=@('大工カードの「この大工に応援要請」をタップ','工事場所・日程（空き日から選択）・費用負担を入力','相手が承認すると、次のステップに進みます');
     n='ステップよん、応援要請です。依頼したい大工のカードから、応援要請ボタンをタップし、工事場所と日程、駐車料金や宿泊費などの費用負担を入力して送信します。日程は、その大工の空き日からのみ選べる仕組みです。相手の工務店が承認すると、条件のやり取りと電子契約の締結に進みます。' },
  @{ k='ステップ 5'; t='条件のやり取り（チャット）'; shot='chat';
     l=@('承認後、当事者だけのチャットで詳細を相談','写真・PDF・地図のURLも送れます','延長・変更依頼や取引の解除もここから');
     n='ステップご、条件のやり取りです。承認されたら、当事者だけが見られるチャットで、詳しい住所や日当、支払い条件を相談してください。写真やPDF、地図のURLも送れます。日程の延長や変更の依頼、取引の解除も、この画面から行えます。' },
  @{ k='ステップ 6'; t='電子請負契約の締結'; shot='contract';
     l=@('条件が固まったら、電子請負契約を締結します','請負代金・現場責任者などを確認し、双方が電子署名','双方の署名がそろうと、取引成立となります');
     n='ステップろく、電子請負契約の締結です。条件が固まったら、請負内容や請負代金、現場責任者などを確認し、発注者と受注者の双方がアプリ上で電子署名します。双方の署名がそろうと、契約が成立し、取引成立となります。本アプリの応援は常に請負契約として行われ、建設業務への労働者派遣には該当しません。職人への指揮命令は、職人が所属する工務店が行います。' },
  @{ k='ステップ 7'; t='完了後の評価'; shot='reqtab';
     l=@('工期が終わった取引は5段階で評価','大工と相手工務店の両方を評価できます','評価は完了した取引のみ。解除した取引は不可');
     n='ステップなな、評価です。工期が終わった取引は、応援タブから、大工と相手の工務店を五段階で評価します。評価できるのは、仕事が完了した取引だけです。評価は利用者全員の平均として表示され、次のマッチングの信頼につながります。' },
  @{ k='ステップ 8'; t='お知らせ・通知'; shot='inbox';
     l=@('ベルマークに未対応の項目が集まります','「通知をオンにする」で新着を音でもお知らせ','反映が遅いときは「更新」ボタンで最新化');
     n='ステップはち、お知らせと通知です。画面上のベルマークを開くと、届いた応援要請や新着メッセージなど、未対応の項目がまとめて表示されます。通知をオンにすれば、新着を音でもお知らせします。' },
  @{ cover=$true; k='さっそく使ってみましょう'; t='困ったときはヘルプ';
     l=@('使い方はアプリ内の「ヘルプ」からいつでも検索できます。','体験モード（URLに ?demo=1）なら、データが24時間で','消えるお試し環境で練習できます。');
...
$value = @(
  ...
  @{ k='課題'; t='職人の繁閑の波';
     ...
     n='工務店には、繁閑の波があります。繁忙期はひとでが足りず工期に追われる一方で、閑散期は腕のいい大工を遊ばせてしまう。応援を探すにも電話頼みで、誰がいつ空いているのかが見えません。これが現場の実情ではないでしょうか。' },
  @{ k='解決'; t='空き状況をリアルタイム共有'; shot='find';
     l=@('参加工務店の大工の空き日がカレンダーで見える','得意作業・資格・日給・★評価も一覧','スマホのホーム画面からアプリとして使えます');
     n='大工シェアネットワークなら、参加工務店の大工の空き日が、カレンダーでリアルタイムに確認できます。得意作業や資格、日給、技術力の評価まで一覧できるため、電話をかけまわす必要はありません。' },
  @{ k='メリット 1'; t='借りる側：すぐに応援を確保'; shot='reqform';
     l=@('空いている大工に、アプリから直接応援要請','★評価と得意作業で、安心して選べる','費用負担も最初に確認できてトラブル防止');
     n='借りる側のメリットです。空いている大工に、アプリから直接応援要請を送れるため、急な工事にもすぐ対応できます。利用者全員の星評価と得意作業を確認したうえで選べるので安心です。駐車料金や宿泊費などの条件も最初に確認でき、トラブルを防げます。' },
  @{ k='メリット 2'; t='貸す側：空きを仕事に変える'; shot='team';
     l=@('空き日を登録しておくだけで依頼が届く','閑散期も職人の仕事と収入を確保','いい仕事は★評価になり、次の依頼につながる');
     n='貸す側のメリットです。大工の空き日を登録しておくだけで、他の工務店から依頼が届きます。閑散期も職人の仕事と収入を確保でき、大切な職人を手放さずにすみます。誠実な仕事は評価となり、次の依頼につながります。' },
  @{ k='安心の仕組み'; t='当事者間の直接取引'; shot='chat';
     l=@('労働条件・支払いは当事者チャットで直接交渉','評価は完了した取引のみ。自作自演はできません','管理者は取引に関与せず、紹介料もありません');
     n='安心の仕組みです。労働条件や支払いは、当事者だけが見られるチャットで直接交渉します。評価は仕事が完了した取引に限られるため、自作自演はできません。管理者は取引に関与せず、紹介料もかかりません。' },
  @{ k='法令順守'; t='応援は、常に請負契約です'; shot='contract';
     l=@('建設業務への労働者派遣は、法律で禁止されています','本アプリの応援は、常に請負（業務委託）契約です','職人への指揮命令は、職人が所属する工務店が行います');
     n='法令面についてもご説明します。建設業務への労働者派遣は、法律で禁止されています。本アプリを通じた応援は、常に請負契約、つまり業務委託契約として行われ、労働者派遣には該当しません。職人への作業指示や労務管理は、職人が所属する工務店が自ら行い、依頼先の工務店が直接指示することはありません。承認後は、双方の電子署名による請負契約の締結を必須としており、法令に沿った形で、安心して地域の職人をシェアしていただけます。' },
...
$caution = @(
  ...
  @{ k='注意点 1'; t='依頼できるのは「空き」の日だけ'; shot='reqform';
     ...
     n='一つ目です。応援要請で選べる日程は、その大工が空きに設定した日だけです。応相談の日は確定ではありませんので、チャットでよくご相談ください。無理な日程を指定して依頼することはできない仕組みになっています。' },
  @{ k='注意点 2'; t='費用負担は具体的に伝える'; shot='reqform';
     l=@('駐車料金・宿泊費・交通費・有料道路の負担を入力','「要相談」のままにせず、できるだけ具体的に','あとからのトラブル防止につながります');
     n='二つ目は、費用負担の確認です。駐車料金や宿泊費、交通費、有料道路の負担者を、要請の際に入力します。要相談のままにせず、できるだけ具体的に伝えることで、あとからのトラブルを防げます。' },
  @{ k='注意点 3'; t='条件のやり取りで詳細を詰める'; shot='chat';
     l=@('承認後は、当事者だけのチャットで詳細を相談','住所・日当・作業内容は必ず具体的に確認','写真やPDFも送れます');
     n='三つ目です。承認されたら、当事者だけが見られるチャットで、詳しい住所や日当、作業内容を具体的に相談してください。写真やPDFも送れますので、現場の情報共有にご活用ください。' },
  @{ k='注意点 4（重要）'; t='電子請負契約を必ず締結する'; shot='contract';
     l=@('承認後は、必ず電子請負契約を締結してください','請負代金・現場責任者を確認し、双方が電子署名','職人への指揮命令は、所属する工務店が行います');
     n='四つ目、とても大切な点です。承認後は、必ず電子請負契約を締結してください。請負代金や現場責任者などの内容を確認し、双方が電子署名することで契約が成立します。職人への作業指示や労務管理は、職人が所属する工務店が行うものとし、依頼した工務店が直接指示することはできません。これは、建設業務への労働者派遣が法律で禁止されているためで、本アプリの応援は常に請負契約として行われる仕組みになっています。' },
  @{ k='注意点 5'; t='予約とスケジュール変更のルール'; shot='reqtab';
     l=@('確定した日は「予約済み」となり、他社は選べません','予定変更は「延長・予定変更を依頼」から相手に依頼','取引の解除もこの画面からいつでも可能です');
     n='五つ目です。承認された日は自動的に予約済みとなり、他の工務店が同じ日を重ねて予約することはできません。日程を変えたいときは、延長・予定変更を依頼のボタンから相手に依頼してください。条件が合わなくなった場合は、取引の解除もこの画面からいつでも行えます。' },
  @{ k='注意点 6'; t='トラブル対応と契約書の保管';
     l=@('現場での事故・支払い等のトラブルは当事者間で解決','管理者は取引の内容には関与しません','契約書は締結後、必ず保存・印刷して保管してください');
     n='最後、六つ目です。現場での事故や、報酬の支払いなどのトラブルは、当事者どうしで解決していただく仕組みです。管理者は取引の内容には関与しません。締結した契約書は、必ずダウンロードまたは印刷して、大切に保管してください。' },

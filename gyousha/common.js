...
(function (global) {
  ...
  // ---- 改ざん検知（電子署名の証拠力強化） ----
  // 契約内容を正規化した文字列。両者はこの内容に署名する。
  H.contentString = function (c) {
    c = c || {};
    return ["title=" + (c.title || ""), "site=" + (c.site || ""), "content=" + (c.content || ""),
      "amount=" + (c.amount || ""), "taxRate=" + (c.taxRate || ""), "taxMode=" + (c.taxMode || "excl"),
      "pay=" + (c.payTerm || ""), "work=" + ((c.workDates || []).join(",")),
      "handover=" + (c.handover || ""), "advance=" + (c.advance || ""), "nowork=" + (c.nowork || ""),
      "note=" + (c.note || "")].join("\n");
  };
  // 契約内容のSHA-256（16進）。署名時に記録し、後から改ざんを検知できる。Promiseを返す。
  H.contentHash = function (c) {
    try {
      var enc = new TextEncoder().encode(H.contentString(c));
      return crypto.subtle.digest("SHA-256", enc).then(function (buf) {
        return Array.prototype.map.call(new Uint8Array(buf), function (b) { return ("0" + b.toString(16)).slice(-2); }).join("");
      });
    } catch (e) { return Promise.resolve(""); }
  };
  // 署名に記録されたハッシュと現在の内容を照合（改ざんの有無）。Promise<bool|null>。
  H.verifySign = function (c, sign) {
    if (!sign || !sign.hash) return Promise.resolve(null);
    return H.contentHash(c).then(function (h) { return h === sign.hash; });
  };
  ...
  H.yakkanHTML = function (terms) {
    ...
    var arts = [
      ["第1条（総則）", "本契約は工事の完成を目的とする請負契約であり、注文者及び受注者は、信義に従い誠実にこれを履行する。本工事は労働者派遣ではなく、受注者は自己の責任と裁量により施工し、その使用する労働者・職人に対する指揮命令、労務管理及び安全衛生管理を自ら行う。"],
      ["第2条（用語の定義）", "本約款において「注文書」とは表面（本注文書兼注文請書）をいい、「本契約」とは注文書及び本約款をいい、「目的物」とは本工事の完成物をいう。"],
      ["第3条（権利義務の譲渡等の制限）", "当事者は、相手方の書面による承諾を得なければ、本契約上の地位又は本契約から生じる権利義務を第三者に譲渡し、承継させ、又は担保に供してはならない。"],
      ["第4条（一括下請負の禁止）", "受注者は、建設業法第22条の定めに従い、本工事の全部又はその主たる部分を、一括して他人に請け負わせてはならない。"],
      ["第5条（再委託）", "受注者は、本工事の一部を第三者に委託しようとするときは、あらかじめ注文者の書面による承諾を得る。この場合も、受注者は当該第三者の行為について注文者に対して責任を負う。"],
      ["第6条（施工体制）", "受注者は、本工事を適切に施工するため、法令に基づく主任技術者その他必要な技術者・現場代理人を配置し、施工体制を整える。"],
      ["第7条（工事用地の確保等）", "注文者は、工事の施工上必要な用地を受注者が使用できるよう確保する。用地の確保に関する費用の負担は、注文書に定めがなければ注文者の負担とする。"],
      ["第8条（設計図書等）", "本工事は、注文書及び注文者の指示する設計図書・仕様に従って施工する。これらの内容に矛盾又は疑義があるときは、受注者は速やかに注文者に通知し、両者協議して定める。"],
      ["第9条（支給材料・貸与機械）", "注文者が材料を支給し、又は建設機械その他の機械器具を貸与する場合の品目、数量、時期、場所、費用の負担及び返還方法は、注文書の記載又は別途両者の書面による合意による。（建設業法第19条第1項第10号）"],
      ["第10条（施工・安全衛生・法令遵守）", "受注者は、建設業法、労働安全衛生法、社会保険及び労働保険に関する法令その他の関係法令及び工事の安全基準を遵守し、災害の防止に努めて施工する。"],
      ["第11条（第三者に対する損害）", "工事の施工について第三者に損害を及ぼしたときは、その賠償は、責めに帰すべき事由のある当事者が負担する。責めに帰すべき事由が双方にあるときは、その割合に応じて負担する。（同項第9号）"],
      ["第12条（設計変更・工期の変更等）", "注文者が設計の変更、工事着手の延期又は工事の全部若しくは一部の中止を求めたときは、両者は協議のうえ、工期の変更、請負代金の額の変更又は損害の負担及びその額の算定方法を定める。（同項第6号）"],
      ["第13条（不可抗力）", "天災地変その他両者の責めに帰することができない事由により、工期の変更又は損害が生じたときは、両者は協議のうえ、その損害の負担及び額を定める。（同項第7号）"],
      ["第14条（物価の変動）", "工期内に賃金水準又は物価の著しい変動が生じ、請負代金の額が不適当となったときは、両者は協議のうえ、請負代金の額を変更することができる。（同項第8号）"],
      ["第15条（完成・検査）", "受注者は、工事を完成したときは注文者に通知し、注文者は、注文書に定める時期及び方法（定めのないときは通知後遅滞なく）により検査を行う。検査に合格しないときは、受注者は自己の費用で速やかに補修し、再検査を受ける。（同項第11号）"],
      ["第16条（引渡し・所有権）", "検査に合格したときは、受注者は目的物を引き渡し、注文者はこれを受領する。目的物の所有権は、引渡しの時に注文者に移転する。"],
      ["第17条（部分使用・部分引渡し）", "注文者は、必要があるときは、受注者の同意を得て、工事の完成前に目的物の一部を使用し、又は部分引渡しを受けることができる。この場合の代金その他の取扱いは両者協議して定める。"],
      ["第18条（請負代金の支払）", "請負代金は、注文書に定める支払時期及び方法により支払う。前払金又は部分払（出来形払）の定めがあるときは、その定めによる。（同項第5号・第12号）"],
      ["第19条（契約不適合責任）", "引き渡された目的物が種類又は品質に関して契約の内容に適合しないときは、注文者は、引渡しの日から" + yrs + "年以内にその旨を通知することにより、受注者に対し、履行の追完（修補等）、代金の減額、損害の賠償又は契約の解除を請求することができる。（同項第13号）"],
      ["第20条（履行遅滞・遅延損害金）", "当事者がその債務の履行を遅滞したときは、遅延損害金その他これによって生じた損害の負担及びその額は、両者が協議のうえ定める。（同項第14号）"],
      ["第21条（期限の利益の喪失）", "当事者の一方について、支払停止、手形・小切手の不渡り、差押え、破産・民事再生等の申立てその他信用を著しく害する事由が生じたときは、その者は相手方の催告を要せず本契約上の債務について期限の利益を失う。"],
      ["第22条（相殺）", "当事者は、相手方に対して有する債権をもって、相手方に対して負う本契約上の債務と相殺することができる。"],
      ["第23条（契約の解除）", "当事者の一方が本契約に違反し、相手方が相当の期間を定めて催告してもその期間内に是正されないときは、相手方は本契約を解除することができる。前条の事由その他契約を継続し難い重大な事由があるときは、催告を要せず直ちに解除することができる。"],
      ["第24条（反社会的勢力の排除）", "当事者は、自己が暴力団その他の反社会的勢力でないこと等を表明・確約し、相手方がこれに反したときは、催告を要せず直ちに本契約を解除することができる。"],
      ["第25条（秘密保持・個人情報）", "当事者は、本契約により知り得た相手方の営業上・技術上の秘密及び個人情報を、本契約の目的以外に使用せず、正当な理由なく第三者に漏らしてはならない。"],
      ["第26条（紛争の解決・合意管轄）", "本契約に関して紛争が生じたときは、両者は誠実に協議して解決を図る。協議が調わないときは、" + court + "を第一審の専属的合意管轄裁判所とする。（同項第15号）"],
      ["第27条（定めのない事項）", "本契約及び本約款に定めのない事項は、建設業法その他の関係法令及び中央建設業審議会が定める標準下請契約約款の趣旨に従い、両者が協議して定める。（同項第16号）"]
    ];
  ...
  H.contractDocHTML = function (c, koumutenName, terms) {
    ...
    var sig = function (role, s, coFallback) {
      return '<div class="sig"><div class="r">' + role + "</div>" +
        ...
        '<div class="d">' + (s && s.at ? "署名日時：" + H.fmtYmd(H.ymd(new Date(s.at))) + " " + H.pad2(new Date(s.at).getHours()) + ":" + H.pad2(new Date(s.at).getMinutes()) : "署名日：　　　年　　月　　日") + "</div>" +
        (s && s.hash ? '<div class="d" style="font-size:10px">改ざん検知コード(SHA-256)：<span style="font-family:monospace">' + e(s.hash.slice(0, 24)) + "…</span></div>" : "") + "</div>";
    };
    ...
    return '<!doctype html><html lang="ja"><head><meta charset="utf-8"><title>' + docTitle + '</title><style>' +
      ...
      "・電子署名は、双方が内容（約款を含む）に同意した記録として、署名者名・日時に加え、契約内容のハッシュ値（改ざん検知コード・SHA-256）を記録します（押印は不要）。署名後に内容が変われば当該コードが一致せず、改ざんを検知できます。<br>" +
      "・本電子署名は合意の記録・証拠として用いるものであり、電子署名法上の認定を受けた電子署名（認証局等によるもの）ではありません。より高い証明力が必要な場合は電子契約サービス等をご検討ください。<br>" +
      "・本約款は、中央建設業審議会の標準下請契約約款等を参考にしたひな型です。個別の取引や法改正への適合について、専門家（行政書士・弁護士等）への相談は任意です（必要に応じてご相談ください）。<br>" +
